...
def admin_health():
    ...
    try:
        ...
        body = json.dumps({'status': 'healthy', 'timestamp': ts}).encode('utf-8')
        return Response(body, 200, {'Content-Type': 'application/json'})
    except Exception:
        body = json.dumps({'status': 'unhealthy', 'timestamp': ts}).encode('utf-8')
        return Response(body, 503, {'Content-Type': 'application/json'})
